...
app = Flask(__name__)
app.secret_key = "soomling_gram_secret_v3"
...
def send_sms(phone, code):
    ...
    # Для реального SMS раскомментировать и добавить API ключ:
    # import requests
    # requests.post('https://sms.ru/sms/send', data={'api_id': 'KEY', 'to': phone, 'msg': f'Код: {code}'})
    return True
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input:focus { outline: none; border-color: #ff6b6b; background: rgba(255,255,255,0.2); }
        input::placeholder { color: rgba(255,255,255,0.6); font-size: 16px; }
        button {
            width: 100%;
            padding: 18px;
        ...
        .switch { text-align: center; margin-top: 20px; color: rgba(255,255,255,0.6); font-size: 16px; }
        .switch a { color: #ff6b6b; text-decoration: none; }
...
<body>
    <div class="glass">
        ...
        <form method="POST" action="/send_sms_code">
            <input type="tel" name="phone" placeholder="📱 Номер телефона (например 79991234567)" required>
...
<body>
    ...
    <div class="container">
        <div class="sidebar" id="sidebar">
            ...
            <div class="search-box"><input type="text" id="searchInput" placeholder="🔍 Поиск по номеру телефона..."></div>
            <div class="tabs">
        ...
        <div class="chat-area">
            <div class="chat-header">
                ...
                <div class="chat-actions">
                    ...
                    <button id="secretChatBtn" style="display:none;">🔒</button>
                </div>
            </div>
    ...
    <div class="modal" id="settingsModal">
        <div class="modal-content">
            ...
            <div class="modal-buttons">
                <button id="saveProfileBtn" class="save-btn">Сохранить</button>
    ...
    <script>
        ...
        let favorites = {{ favorites_json|safe }};
        let secretChats = {{ secret_chats_json|safe }};
        ...
        function addMessage(text, isSent, time, isFile=false, fileUrl=null, isVoice=false, isSecret=false){
            let container=document.getElementById('messages');
            let div=document.createElement('div');
            div.className=`message ${isSent ? 'sent' : 'received'} ${isSecret ? 'secret' : ''}`;
            let content='';
            if(isFile && fileUrl){
                content=`<div class="file-message">📁 <a href="${fileUrl}" download style="color:white">${text}</a></div>`;
                ...
                content=`<div class="voice-message" onclick="document.getElementById('audioPlayer').src='${fileUrl}'; document.getElementById('audioPlayer').play()">
                    ...
                    <div class="voice-wave"><div class="wave-bar"></div><div class="wave-bar"></div><div class="wave-bar"></div><div class="wave-bar"></div></div>
                    <div>Голосовое сообщение</div>
                </div>`;
            } else {
                content=`<div class="message-bubble">${escapeHtml(text)}</div>`;
            }
            div.innerHTML=content+`<div class="message-time">${time} ${isSecret ? '🔒' : ''}</div>`;
        ...
        function loadUserMessages(phone, name, isSecret=false){
            ...
            document.getElementById('secretChatBtn').style.display = 'block';
            let url = isSecret ? `/get_secret_messages/${phone}` : `/get_messages/${phone}`;
            fetch(url).then(r=>r.json()).then(data=>{
                let c=document.getElementById('messages');
                c.innerHTML='';
                data.forEach(msg=>addMessage(msg[1], msg[0]===myPhone, msg[2], msg[3], msg[4], msg[5], isSecret));
            });
            if(window.innerWidth<=768) closeSidebar();
            fetch(`/get_user_status/${phone}`).then(r=>r.json()).then(d=>{
                document.getElementById('chatUserStatus').innerHTML = d.online ? '🟢 Онлайн' : '⚫ Офлайн';
            });
        }
        ...
        function loadGroupMessages(id, name){
            ...
            document.getElementById('secretChatBtn').style.display = 'none';
            fetch(`/get_group_messages/${id}`).then(r=>r.json()).then(data=>{
                let c=document.getElementById('messages');
                c.innerHTML='';
                data.forEach(msg=>addMessage(`${msg[1]}: ${msg[2]}`, msg[0]===myPhone, msg[3]));
            });
            if(window.innerWidth<=768) closeSidebar();
        }
        ...
        function sendMessage(){
            ...
            if(currentChatType === 'user'){
                ...
                socket.emit('send_message', {to: currentChat, message: text, is_secret: isSecret});
                addMessage(text, true, new Date().toLocaleTimeString(), false, null, false, isSecret);
        ...
        function sendFile(file, isVoice=false){
            ...
            fetch('/upload_file', {method:'POST', body:fd}).then(r=>r.json()).then(data=>{
                if(data.url){
                    ...
                    addMessage(file.name, true, new Date().toLocaleTimeString(), true, data.url, isVoice);
                }
            });
        }
        ...
        // Секретный чат
        document.getElementById('secretChatBtn').onclick = () => {
            if(currentChat && currentChatType==='user'){
                fetch('/start_secret_chat', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({with_user:currentChat})}).then(()=>{
                    loadUserMessages(currentChat, currentChatName, true);
                });
            }
        };
        document.getElementById('secretTabBtn').onclick = () => {
        ...
        socket.on('receive_message', (data) => {
            if(currentChatType === 'user' && currentChat === data.from){
                addMessage(data.message, false, data.timestamp, data.is_file, data.file_url, data.is_voice, data.is_secret);
            }
        });
        socket.on('receive_group_message', (data) => {
            if(currentChatType === 'group' && currentChat == data.group_id){
                addMessage(`${data.from_name}: ${data.message}`, false, data.timestamp);
            }
        });

        // Выбор чатов
        function selectUser(phone, name){ loadUserMessages(phone, name, secretChats.includes(phone)); }
        ...
        document.getElementById('saveProfileBtn').onclick = () => {
            ...
            fetch('/update_profile', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({first_name:fn, last_name:ln})}).then(()=>location.reload());
        };
        ...
        document.getElementById('giftBtn').onclick = () => {
            fetch('/get_gifts').then(r=>r.json()).then(data=>{
                ...
                data.gifts.forEach(g=>{
                    ...
                    div.innerHTML=`<div style="font-size:36px">${g.emoji}</div><div><div style="font-size:16px">${g.name}</div><div style="font-size:14px">${g.price} монет</div></div>`;
                    div.onclick=()=>{
...
def complete_register():
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO users (phone, email, first_name, last_name, password, secret_key) VALUES (?, ?, ?, ?, ?, ?)", (phone, email, first_name, last_name, password, secret_key))
        conn.commit()
        session['phone'] = phone
...
def chat():
    ...
    coins = c.fetchone()[0]
    # Секретные чаты
    c.execute("SELECT secret_key FROM users WHERE phone=?", (session['phone'],))
    secret_key = c.fetchone()[0]
    c.execute("SELECT phone, first_name, last_name FROM users WHERE secret_key != ?", (secret_key,))
    secret_users = c.fetchall()
    conn.close()
    return render_template_string(CHAT_PAGE, users=users, groups=groups, favorites=fav_users, favorites_json=json.dumps(favs), secret_chats=secret_users, secret_chats_json=json.dumps([u[0] for u in secret_users]), my_phone=session['phone'], my_name=f"{session['first_name']} {session['last_name']}", first_name=session['first_name'], last_name=session['last_name'], coins=coins)

@app.route('/get_messages/<to_phone>')
def get_messages(to_phone):
    conn = sqlite3.connect('soomlinggram.db')
    c = conn.cursor()
    c.execute("SELECT from_user, message, timestamp, is_file, file_url, is_voice FROM messages WHERE ((from_user=? AND to_user=?) OR (from_user=? AND to_user=?)) AND is_secret=0 ORDER BY id", (session['phone'], to_phone, to_phone, session['phone']))
    msgs = c.fetchall()
    conn.close()
    return jsonify(msgs)
...
def get_secret_messages(to_phone):
    conn = sqlite3.connect('soomlinggram.db')
    c = conn.cursor()
    c.execute("SELECT from_user, message, timestamp, is_file, file_url, is_voice FROM messages WHERE ((from_user=? AND to_user=?) OR (from_user=? AND to_user=?)) AND is_secret=1 ORDER BY id", (session['phone'], to_phone, to_phone, session['phone']))
    msgs = c.fetchall()
    conn.close()
    return jsonify(msgs)
...
def get_group_messages(group_id):
    conn = sqlite3.connect('soomlinggram.db')
    c = conn.cursor()
    c.execute("SELECT from_user, from_name, message, timestamp FROM group_messages WHERE group_id=? ORDER BY id", (group_id,))
    msgs = c.fetchall()
    conn.close()
    return jsonify(msgs)
...
@app.route('/start_secret_chat', methods=['POST'])
def start_secret_chat():
    with_user = request.json.get('with_user')
    conn = sqlite3.connect('soomlinggram.db')
    c = conn.cursor()
    # Создаём секретный ключ для чата
    c.execute("UPDATE users SET secret_key=? WHERE phone=?", (generate_secret_key(), session['phone']))
    conn.commit()
    conn.close()
    return jsonify({'status': 'ok'})
...
def handle_send_message(data):
    ...
    conn = sqlite3.connect('soomlinggram.db')
    c = conn.cursor()
    c.execute("INSERT INTO messages (from_user, to_user, message, timestamp, is_file, file_url, is_voice, is_secret) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (session['phone'], to, msg, datetime.now().isoformat(), 1 if is_file else 0, file_url, 1 if is_voice else 0, 1 if is_secret else 0))
    conn.commit()
    conn.close()
    emit('receive_message', {'from': session['phone'], 'message': msg, 'timestamp': datetime.now().strftime("%H:%M:%S"), 'is_file': is_file, 'file_url': file_url, 'is_voice': is_voice, 'is_secret': is_secret}, room=to)
...
def handle_send_group_message(data):
    ...
    conn = sqlite3.connect('soomlinggram.db')
    c = conn.cursor()
    c.execute("INSERT INTO group_messages (group_id, from_user, from_name, message, timestamp, is_file, file_url) VALUES (?, ?, ?, ?, ?, ?, ?)", (group_id, session['phone'], f"{session['first_name']} {session['last_name']}", msg, datetime.now().isoformat(), 1 if is_file else 0, file_url))
    conn.commit()
    c.execute("SELECT members FROM groups WHERE id=?", (group_id,))
    members = json.loads(c.fetchone()[0])
    conn.close()
    for member in members:
        emit('receive_group_message', {'group_id': group_id, 'from_name': f"{session['first_name']} {session['last_name']}", 'message': msg, 'timestamp': datetime.now().strftime("%H:%M:%S")}, room=member)
